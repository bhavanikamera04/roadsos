...
export interface Hospital {
  id: string
  name: string
  location: { lat: number, lon: number } | string // Supabase returns geography as string
  trauma_score: number
  has_icu: boolean
  is_24hr: boolean
  level: number
  ...
  return hospitals
    .map(hospital => {
      ...
      if (typeof hospital.location === 'string') {
        // Parse PostGIS POINT(lon lat)
        const coords = hospital.location.replace('POINT(', '').replace(')', '').split(' ');
